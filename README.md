...
The cursor is the state. While everything is running there is no cursor, just
the prompt; the moment an agent stops, the cursor appears where your answer
would go.

It is drawn rather than taken from the font, which has no chevron with the
weight of a prompt: the typographic one is a thin ornament and the bold one is
a navigation arrow.
